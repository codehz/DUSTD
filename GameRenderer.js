System.register(["immutable", "Machine", "SpriteLoader", "./Blocks", "./Enemy", "./FontLoader", "./global", "./Scene", "./ST"], function (exports_1, context_1) {
    "use strict";
    var immutable_1, Machine_1, SpriteLoader_1, Blocks_1, Enemy_1, FontLoader_1, global_1, Scene_1, ST_1, ActualMap, CachedMap, BaseMachineMap, InfoBoard, MachineSelector, FloatItem, GameRenderer;
    ...
    return {
        ...
        execute: function () {
            ...
            MachineSelector = class MachineSelector extends Scene_1.BaseEntity {
                ...
                mouse(evt) {
                    const { x, y } = evt;
                    if (evt.type === Scene_1.XMouseEventType.pointerdown && x >= 8 && y >= 10 && y < 62) {
            ...
            FloatItem = class FloatItem extends Scene_1.BaseEntity {
                ...
                mouse(evt) {
                    ...
                    if (this.mac) {
                        ...
                        if (evt.type === Scene_1.XMouseEventType.pointerup) {
                            this.hook(this.mac);
                            this.mac = null;
                        }
                    }
            ...
            GameRenderer = class GameRenderer extends Scene_1.BaseEntity {
                constructor(map) {
                    super(true);
                ...
                mayPlace(mac) {
                    this.willPlace = mac;
                    setTimeout((x) => this.willPlace = null);
                }
                measure(limit) {
                ...
                render(base) {
                    ...
                    for (let i = (-hb / 2 - 2) | 0; i < hb / 2 + 1; i++) {
                        for (let j = (-wb / 2 - 2) | 0; j < wb / 2 + 1; j++) {
                            ...
                            if (v) {
                                FontLoader_1.textLoader.drawLine(v.toFixed(1), Object.assign({}, pos, { y: pos.y + blocksize / 4, scale: 1 / 4 }));
                            }
                        }
                    }
                ...
                mouse(ev) {
                    ...
                    switch (ev.type) {
                        case Scene_1.XMouseEventType.pointerdown:
                            this.spos = {
                                x: ev.x,
                                y: ev.y,
                            };
                            break;
                        case Scene_1.XMouseEventType.pointermove:
                            if (this.spos) {
                            ...
                            break;
                        case Scene_1.XMouseEventType.pointerup:
                            if (this.willPlace) {
                                this.map.tryPlace({ x: bx, y: by }, this.willPlace);
                            }
                            this.st.emit('focused', this);
                        case Scene_1.XMouseEventType.pointerleave:
                            if (this.spos) {
                                this.lst.emitFn('offset', ({ x, y }) => ({
                                    x: x - ev.x + this.spos.x,
                                    y: y - ev.y + this.spos.y,
                            ...
                            break;
                        case Scene_1.XMouseEventType.wheel:
            ...
            exports_1("GameRenderer", GameRenderer);
        }
    };
});
